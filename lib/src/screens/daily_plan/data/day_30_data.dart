Map<int, Map<String, List<Map<String, String?>>>> all30DayData = {
  1: {
    'quran': [
      {
        'surahNumber': '1',
        'start': '1',
        'end': null,
      },
      {
        'surahNumber': '2',
        'start': '1',
        'end': '141',
      }
    ],
    'hadith': [
      {
        'translation':
            '''Whoever seeks forgiveness for every male and female believer, Allah will record a good deed for him for every male and female believer.
[Tabarani]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''رَبَّنَا اغْفِرْ لِي وَلِوَالِدَيَّ وَلِلْمُؤْمِنِينَ يَوْمَ يَقُومُ الْحِسَابُ''',
        'translation':
            '''Our Lord, Forgive me, and my parents, and the believers on the Day of Accounting.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ اجْعَلْ صِيامي فيهِ صِيامَ الصّائِمينَ، وَقِيامي فيهِ قيامَ الْقائِمينَ، وَنَبِّهْني فيهِ عَنْ نَوْمَةِ الْغافِلينَ، وَهَبْ لى جُرْمي فيهِ يا اِلـهَ الْعالَمينَ، وَاعْفُ عَنّي يا عافِياً عَنْ الْمجْرِمينَ''',
        'translation':
            '''ALLAH, on this day make my fasts the fasts of those who fast (sincerely), and my standing up in prayer of those who stand up in prayer (obediently), awaken me in it from the sleep of the heedless, and forgive me my sins , O God of the worlds, and forgive me, O one who forgives the sinners.''',
      }
    ]
  },
  2: {
    'quran': [
      {
        'surahNumber': '2',
        'start': '142',
        'end': '252',
      }
    ],
    'hadith': [
      {
        'translation':
            '''Whoever recites a letter from the Book of Allah, he will receive one good deed as ten good deeds like it. I do not say that Alif Lam Meem is one letter, but rather Alif is a letter, Lam is a letter, and Meem is a letter.
[Tirmidhi]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنَ الْجُبْنِ، وَأَعُوذُ بِكَ أَنْ أُرَدَّ إِلَى أَرْذَلِ الْعُمُرِ، وَأَعُوذُ بِكَ مِنْ فِتْنَةِ الدُّنْيَا، وَأَعُوذُ بِكَ مِنْ عَذَابِ الْقَبْرِ''',
        'translation':
            '''O Allah! I seek refuge in You from cowardice. O Allah, I seek refuge in You from being brought back to a bad stage of the age, I seek refuge in You from trials of this world and from the torment of the grave.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ قَرِّبْني فيهِ اِلى مَرْضاتِكَ، وَجَنِّبْني فيهِ مِنْ سَخَطِكَ وَنَقِماتِكَ، وَوَفِّقْني فيهِ لِقِرآءَةِ آياتك بِرَحْمَتِكَ يا اَرْحَمَ الرّاحِمينَ''',
        'translation':
            '''ALLAH, on this day, take me closer towards Your pleasure, keep me away from Your anger and punishment, grant me the opportunity to recite Your verses (of the Qur’an), by Your mercy, O the most Merciful.''',
      }
    ]
  },
  3: {
    'quran': [
      {
        'surahNumber': '2',
        'start': '253',
        'end': null,
      },
      {
        'surahNumber': '3',
        'start': '1',
        'end': '92',
      }
    ],
    'hadith': [
      {
        'translation':
            '''If you loan Allah a goodly loan, He will multiply it for you and forgive you. And Allah is [most] Appreciative and Forebearing.
[At-Taghabun, 64:17]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنْ عَذَابِ جَهَنَّمَ وَمِنْ عَذَابِ الْقَبْرِ وَمِنْ فِتْنَةِ
الْمَحْيَا وَالْمَمَاتِ وَمِنْ شَرِّ فِتْنَةِ الْمَسِيحِ الدَّجَّالِ''',
        'translation':
            '''O Allah! I seek refuge in you from the torment of hell, from the torment of the grave, from the trial of life and death and from the evil of the trial of Masih al-Dajjal.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ ارْزُقْني فيهِ الذِّهْنَ وَالتَّنْبيهَ ، وَباعِدْني فيهِ مِنَ السَّفاهَةِ وَالَّتمْويهِ ، وَاجْعَلْ لى نَصيباً مِنْ كُلِّ خَيْر تُنْزِلُ فيهِ، بِجُودِكَ يا أجود الاْجْوَدينَ''',
        'translation':
            '''ALLAH, on this day, grant me wisdom and awareness, keep me away from foolishness and pretension, grant me a share in every blessing You send down, by You generosity, O the most Generous.''',
      }
    ]
  },
  4: {
    'quran': [
      {
        'surahNumber': '3',
        'start': '93',
        'end': null,
      },
      {
        'surahNumber': '4',
        'start': '1',
        'end': '23',
      }
    ],
    'hadith': [
      {
        'translation':
            '''He who supplicates for his brother behind his back (in his absence), the Angel commissioned (for carrying supplication to his Lord) says: Amen, and it is for you also.
[Muslim]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''رَبِّ تَقَبَّلْ تَوْبَتِي وَاغْسِلْ حَوْبَتِي وَأَجِبْ دَعْوَتِي وَثَبِّتْ حُجَّتِي
وَاهْدِ قَلْبِي وَسَدِّدْ لِسَانِي وَاسْلُلْ سَخِيمَةَ قَلْبِي''',
        'translation':
            '''My Lord, accept my repentance, wash away my sin, answer my supplication, clearly establish my evidence, guide my heart, make true my tongue and draw out malice in my breast.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ قَوِّني فيهِ عَلى إقامة أمرك، وأذقني فيهِ حَلاوَةَ ذِكْرِكَ، وَاَوْزعْني فيهِ لاِداءِ شُكْرِكَ بِكَرَمِكَ، وَاحْفَظْني فيهِ بِحِفْظِكَ وَسَتْرِكَ، يا أبصر النّاظِرينَ''',
        'translation':
            '''ALLAH, on this day, strengthen me in carrying out Your commands, let me taste the sweetness of Your remembrance, grant me, through Your graciousness, that I give thanks to You. Protect me, with Your protection and cover, O the most discerning of those who see.''',
      }
    ]
  },
  5: {
    'quran': [
      {
        'surahNumber': '4',
        'start': '24',
        'end': '147',
      }
    ],
    'hadith': [
      {
        'translation':
            '''There is no servant who fasts a day in the way of Allah except that Allah will distance his face from the Hellfire by a length of seventy years of travel.
[Muslim]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''الْحَمْدُ لِلَّهِ الَّذِي أَطْعَمَنِي هَذَا الطَّعَامَ وَرَزَقَنِيهِ مِنْ غَيْرِ حَوْلٍ مِنِّي
وَلَا قُوَّةٍ''',
        'translation':
            '''All praise is due to Allah who has given me food to eat and provided it without any endeavour on my part or any power.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ اجْعَلْني فيهِ مِنْ الْمُسْتَغْفِرينَ، وَاجْعَلْني فيهِ مِنْ عِبادِكَ الصّالِحينَ اْلقانِتينَ ، وَاجْعَلني فيهِ مِنْ اَوْلِيائِكَ الْمُقَرَّبينَ، بِرَأْفَتِكَ يا اَرْحَمَ الرّاحِمينَ''',
        'translation':
            '''ALLAH, on this day, place me among those who seek forgiveness. Place me among Your righteous and obedient servants, and place me among Your close friends, by Your kindness, O the most Merciful.''',
      }
    ]
  },
  6: {
    'quran': [
      {
        'surahNumber': '4',
        'start': '148',
        'end': null,
      },
      {
        'surahNumber': '5',
        'start': '1',
        'end': '81',
      }
    ],
    'hadith': [
      {
        'translation': '''The best of you is the one who has the best character.
[Al Bukhari]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ اهْدِنِي لِأَحْسَنِ الْأَعْمَالِ وَأَحْسَنِ الْأَخْلَاقِ لَا يَهْدِي لِأَحْسَنِهَا
إِلَّا أَنْتَ وَقِنِي سَيِّئَ الْأَعْمَالِ وَسَيِّئَ الْأَخْلَاقِ لَا يَقِي سَيِّئَهَا إِلَّا أَنْتَ''',
        'translation':
            '''O Allah, guide me to the best of deeds and the best of manners, for none can guide to the best of them but You. And protect me from bad deeds and bad manners, for none can protect against them but You.''',
      },
      {
        'arabic':
            '''اللّـهُمَّ لا تَخْذُلْني فيهِ لِتَعَرُّضِ مَعْصِيَتِكَ، وَلا تَضْرِبْني بِسِياطِ نَقِمَتِكَ، وَزَحْزِحْني فيهِ مِنْ مُوجِباتِ سَخَطِكَ ، بِمَنِّكَ وأياديك يا مُنْتَهى رَغْبَةِ الرّاغِبينَ''',
        'translation':
            '''ALLAH, on this day, do not let me abase myself by incurring Your disobedience, and do not strike me with the whip of Your punishment, keep me away from the causes of Your anger, by and Your power, O the ultimate wish of those who desire.''',
      }
    ]
  },
  7: {
    'quran': [
      {
        'surahNumber': '5',
        'start': '82',
        'end': null,
      },
      {
        'surahNumber': '6',
        'start': '1',
        'end': '110',
      }
    ],
    'hadith': [
      {
        'translation':
            '''The most beloved people to Allah are those who are most beneficial to the people.
[Ahmad]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''رَبِّ أَوْزِعْنِي أَنْ أَشْكُرَ نِعْمَتَكَ الَّتِي أَنْعَمْتَ عَلَيَّ وَعَلَىٰ وَالِدَيَّ وَأَنْ
أَعْمَلَ صَالِحًا تَرْضَاهُ وَأَدْخِلْنِي بِرَحْمَتِكَ فِي عِبَادِكَ الصَّالِحِينَ''',
        'translation':
            '''O my Lord! Inspire and bestow upon me the power and ability that I may be grateful for Your
favours which You have bestowed on me and on my parents, and that I may do good deeds
(righteous) that will please You, and admit me by Your mercy among Your righteous
servants.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ اَعِنّي فِيهِ عَلى صِيامِهِ وَقِيامِهِ، وَجَنِّبْني فيهِ مِنْ هَفَواتِهِ وَآثامِهِ، وَارْزُقْني فيهِ ذِكْرَكَ بِدَوامِهِ، بِتَوْفيقِكَ يا هادِيَ الْمُضِلّينَ''',
        'translation':
            '''ALLAH, on this day, help me with its fasts and prayers, and keep me away from mistakes and sins of the day, grant me that I remember You continuously through the day, by Your assistance, O the Guide of those who stray.''',
      }
    ]
  },
  8: {
    'quran': [
      {
        'surahNumber': '6',
        'start': '111',
        'end': null,
      },
      {
        'surahNumber': '7',
        'start': '1',
        'end': '87',
      }
    ],
    'hadith': [
      {
        'translation':
            '''The closest of people to me (Prophet صلى الله عليه وسلم ) on the Day of Judgement will be those who send the most salah (blessings) to me.
[Tirmidhi]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''رَبِّ اجْعَلْنِي مُقِيمَ الصَّلاَةِ وَمِن ذُرِّيَّتِي رَبَّنَا وَتَقَبَّلْ
دُعَاء''',
        'translation':
            '''O Lord, make me steadfast in prayer, and (also) some of my offspring. O our Lord, accept my prayer.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ ارْزُقْني فيهِ رَحْمَةَ الأيتام، وإطعام اَلطَّعامِ، وإفشاء السَّلامِ، وَصُحْبَةَ الْكِرامِ، بِطَولِكَ يا ملجأ الآملين''',
        'translation':
            '''ALLAH, on this day, let me have mercy on the orphans, and feed [the hungry], and spread peace, and keep company with the noble-minded, O the shelter of the hopeful.''',
      }
    ]
  },
  9: {
    'quran': [
      {
        'surahNumber': '7',
        'start': '88',
        'end': null,
      },
      {
        'surahNumber': '8',
        'start': '1',
        'end': '40',
      }
    ],
    'hadith': [
      {
        'translation':
            '''Harithah bin Wahb narrated that the Messenger of Allah (صلى الله عليه وسلم) said:“Shall I not tell you about the people of Paradise? Every weak and oppressed one. Shall I not tell you about the people of Hell? Every harsh, haughty and arrogant one.”
[Ibn Majah]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنْ مُنْكَرَاتِ الأَخْلاَقِ وَالأَعْمَالِ
وَالأَهْوَاءِ''',
        'translation':
            '''O Allah, I seek refuge in You from evil character, evil actions, and evil desires.''',
      },
      {
        'arabic':
            '''اللّـهُمَّ اجْعَلْ لي فيهِ نَصيباً مِنْ رَحْمَتِكَ الْواسِعَةِ، وَاهْدِني فيهِ لِبَراهينِكَ السّاطِعَةِ، وَخُذْ بِناصِيَتي اِلى مَرْضاتِكَ الْجامِعَةِ، بِمَحَبَّتِكَ يا أمل الْمُشْتاقينَ''',
        'translation':
            '''ALLAH, on this day, grant me a share from Your mercy which is wide, guide me towards Your shining proofs, lead me to Your all encompassing pleasure, by Your love, O the hope of the desirous.''',
      }
    ]
  },
  10: {
    'quran': [
      {
        'surahNumber': '8',
        'start': '41',
        'end': null,
      },
      {
        'surahNumber': '9',
        'start': '1',
        'end': '92',
      }
    ],
    'hadith': [
      {
        'translation':
            '''He who has done a wrong affecting his brother's honour or anything else, let him ask his forgiveness today before the time (i.e., the Day of Resurrection) when he will have neither a dinar nor a dirham. If he has done some good deeds, a portion equal to his wrong doings will be subtracted from them; but if he has no good deeds, he will be burdened with the evil deeds of the one he had wronged in the same proportion.
[Al Bukhari]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''رَّبِّ أَدْخِلْنِي مُدْخَلَ صِدْقٍ وَأَخْرِجْنِي مُخْرَجَ صِدْقٍ
وَاجْعَل لِّي مِن لَّدُنكَ سُلْطَانًا نَّصِيرًا''',
        'translation':
            '''O Lord, cause me to enter a goodly entrance and cause me to exit a goodly exit, and
grant me from Yourself a supporting authority.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ اجْعَلْني فيهِ مِنَ الْمُتَوَكِّلينَ عَلَيْكَ، وَاجْعَلْني فيهِ مِنَ الْفائِزينَ لَدَيْكَ، وَاجْعَلْني فيهِ مِنَ الْمُقَرَّبينَ اِلَيْكَ، بإحسانك يا غايَةَ الطّالِبينَ''',
        'translation':
            '''ALLAH, on this day, make me, among those who rely on You, from those who You consider successful, and place me among those who are near to you, by Your favor, O goal of the seekers.''',
      }
    ]
  },
  11: {
    'quran': [
      {
        'surahNumber': '9',
        'start': '93',
        'end': null,
      },
      {
        'surahNumber': '11',
        'start': '1',
        'end': '5',
      }
    ],
    'hadith': [
      {
        'translation':
            '''This world is a prison for the believer and a paradise for the disbeliever.
[Sunan Ibn Majah]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اَللّٰھُمَّ إِنِّیْ أَعُوْذُبِکَ مِنْ جَھْدِ الْبَلاَءِ ، وَدَرْکِ الشَّقَاءِ،
وَسُوءِ الْقَضَاءِ ، وَشَمَاتَةِ الأَعْدَاءِ''',
        'translation':
            '''O Allah! Indeed I seek refuge in You from severe trials and hardships, being overtaken by misfortune, a bad fate and the rejoicing of enemies.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ حَبِّبْ اِلَيَّ فيهِ الإحسان، وَكَرِّهْ اِلَيَّ فيهِ الْفُسُوقَ وَالْعِصْيانَ، وَحَرِّمْ عَلَيَّ فيهِ السَّخَطَ وَالنّيرانَ بِعَوْنِكَ يا غِياثَ الْمُسْتَغيثينَ''',
        'translation':
            '''ALLAH, on this day, make me love goodness, and dislike corruption and disobedience, bar me from anger and the fire [of Hell], by Your help, O the helper of those who seek help''',
      }
    ]
  },
  12: {
    'quran': [
      {
        'surahNumber': '11',
        'start': '6',
        'end': null,
      },
      {
        'surahNumber': '12',
        'start': '1',
        'end': '52',
      }
    ],
    'hadith': [
      {
        'translation':
            '''Keep your rows straight for your rows resemble those of the angels. Join the shoulders and fill the gaps between yourselves, be gentle and soft in your brother's hands and do not leave the gaps for syaitan. He who connects a row, Allah will connect him (with His mercy).
[Musnad Ahmad]''',
      }
    ],
    'dua': [
      {
        'arabic': '''رَّبِّ اغْفِرْ وَارْحَمْ وَأَنتَ خَيْرُ الرَّاحِمِينَ''',
        'translation':
            '''My Lord! Forgive and have mercy, for You are the best of those who show mercy.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ زَيِّنّي فيهِ بِالسِّتْرِ وَالْعَفافِ، وَاسْتُرْني فيهِ بِلِباسِ الْقُنُوعِ وَالْكَفافِ، وَاحْمِلْني فيهِ عَلَى الْعَدْلِ والإنصاف، وَآمِنّي فيهِ مِنْ كُلِّ ما أخاف، بِعِصْمَتِكَ يا عِصْمَةَ الْخائِفينَ''',
        'translation':
            '''ALLAH, on this day, beautify me with covering and chastity, cover me with the clothes of contentment and chastity, let me adhere to justice and fairness, and keep me safe from all that I fear, by Your protection, O the protector of the frightened.''',
      }
    ]
  },
  13: {
    'quran': [
      {
        'surahNumber': '12',
        'start': '52',
        'end': null,
      },
      {
        'surahNumber': '14',
        'start': '1',
        'end': null,
      }
    ],
    'hadith': [
      {
        'translation':
            '''He who observed the 'Isha' prayer in congregation, it was as if he prayed up to midnight, and he who prayed the morning prayer in congregation, it was as if he prayed the whole night.
[Muslim]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ أَعِنِّي عَلَى ذِكْرِكَ، وَشُكْرِكَ، وَحُسْنِ عِبَادَتِكَ''',
        'translation':
            '''O Allah, help me remember You, to be grateful to You, and to worship You in an excellent manner.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ طَهِّرْني فيهِ مِنَ الدَّنَسِ والأقذار، وَصَبِّرْني فيهِ عَلى كائِناتِ الأقدار، وَوَفِّقْني فيهِ لِلتُّقى وَصُحْبَةِ الأبرار، بِعَوْنِكَ يا قُرَّةَ عَيْنِ الْمَساكينَ''',
        'translation':
            '''ALLAH, on this day, purify me from un-cleanliness and dirt, make me patient over events that are decreed, grant me the ability to be pious, and keep company with the good, by Your help, O the beloved of the destitute.''',
      }
    ]
  },
  14: {
    'quran': [
      {
        'surahNumber': '15',
        'start': '1',
        'end': null,
      },
      {
        'surahNumber': '16',
        'start': '1',
        'end': '128',
      }
    ],
    'hadith': [
      {
        'translation':
            '''And (remember) when your Lord proclaimed, "If you are grateful, I will surely increase you (in favor), but if you deny, indeed, My punishment is severe."
[Ibrahim, 14:7]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''رَبَّنَا آتِنَا فِي الدُّنْيَا حَسَنَةً وَفِي الآخِرَةِ حَسَنَةً وَقِنَا''',
        'translation':
            '''Our Lord, give us in this world [that which is] good and in the Hereafter [that which
is] good and protect us from the punishment of the Fire.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ لا تُؤاخِذْني فيهِ بِالْعَثَراتِ، وَاَقِلْني فيهِ مِنَ الْخَطايا وَالْهَفَواتِ ، وَلا تَجْعَلْني فيهِ غَرَضاً لِلْبَلايا والآفات، بِعِزَّتِكَ يا عِزَّ الْمُسْلِمينَ''',
        'translation':
            '''ALLAH, on this day, do not condemn me for slips, make me decrease mistakes and errors, do not make me a target for afflictions and troubles, by Your honor, O the honor of the Muslims.''',
      }
    ]
  },
  15: {
    'quran': [
      {
        'surahNumber': '17',
        'start': '1',
        'end': null,
      },
      {
        'surahNumber': '18',
        'start': '1',
        'end': '74',
      }
    ],
    'hadith': [
      {
        'translation':
            '''Whoever takes a path upon which to obtain knowledge, Allah makes the path to Paradise easy for him.
[Tirmidhi]

''',
      }
    ],
    'dua': [
      {
        'arabic': '''رَّبِّ زِدْنِى عِلْمًا''',
        'translation': '''O’My Lord! Increase me in knowledge.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ ارْزُقْني فيهِ طاعَةَ الْخاشِعينَ، وَاشْرَحْ فيهِ صَدْري بإنابة الْمخْبِتينَ، بأمانك يا أمان الْخائِفينَ''',
        'translation':
            '''Lord! Grant me in it the obedience of the devout, expand my chest in it with the return to You of those who abandoned You, through Your security, O One Who brings security to the fearful!''',
      }
    ]
  },
  16: {
    'quran': [
      {
        'surahNumber': '18',
        'start': '75',
        'end': null,
      },
      {
        'surahNumber': '20',
        'start': '1',
        'end': null,
      }
    ],
    'hadith': [
      {
        'translation':
            '''The servant is closest to his Lord during prostration, so increase your supplications therein.
[Muslim]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنَ الأَرْبَعِ مِنْ عِلْمٍ لاَ يَنْفَعُ وَمِنْ قَلْبٍ لاَ
يَخْشَعُ وَمِنْ نَفْسٍ لاَ تَشْبَعُ وَمِنْ دُعَاءٍ لاَ يُسْمَعُ
''',
        'translation':
            '''O Allah, I seek refuge in Thee from four things: Knowledge which does not profit, a heart which is not submissive, a soul which has an insatiable appetite, and a supplication which is not heard.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ وَفِّقْني فيهِ لِمُوافَقَةِ الأبرار، وَجَنِّبْني فيهِ مُرافَقَةَ الأشرار، وَآوِني فيهِ بِرَحْمَتِكَ اِلى دارِ الْقَـرارِ، بإلهيّتك يا اِلـهَ الْعالَمينَ''',
        'translation':
            '''ALLAH, on this day, grant me compatibility with the good, keep me away from patching up with the evil, lead me in it, by Your mercy, to the permanent abode, by Your God ship, O the God of the worlds.''',
      }
    ]
  },
  17: {
    'quran': [
      {
        'surahNumber': '17',
        'start': '1',
        'end': null,
      },
      {
        'surahNumber': '22',
        'start': '1',
        'end': null,
      }
    ],
    'hadith': [
      {
        'translation':
            '''Every day two angels come down from Heaven and one of them says, 'O Allah! Compensate every person who spends in Your Cause,' and the other (angel) says, 'O Allah! Destroy every miser.
[Al Bukhari]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اَللَّهُمَّ إِنِّي أَسْأَلُكَ عِلْمًا نَافِعًا, وَ رِزْقًا طَيَّبًا, وَ عَمَلاً
مُتَقَبَّل''',
        'translation':
            '''O Allah! I ask You for knowledge that is of benefit, a good provision and deeds that will be accepted.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ اهْدِني فيهِ لِصالِحِ الإعمال، وَاقْضِ لي فيهِ الْحَوائِجَ والآمال، يا مَنْ لا يَحْتاجُ اِلَى التَّفْسيرِ وَالسُّؤالِ، يا عالِماً بِما في صُدُورِ الْعالَمينَ، صَلِّ عَلى مُحَمَّد وَآلِهِ الطّاهِرينَ''',
        'translation':
            '''ALLAH, on this day, guide me towards righteous actions, fulfill my needs and hopes, O One who does not need explanations nor questions, O One who knows what is in the chests of the (people of the) world. Bless Muhammad and his family, the Pure.''',
      }
    ]
  },
  18: {
    'quran': [
      {
        'surahNumber': '23',
        'start': '1',
        'end': null,
      },
      {
        'surahNumber': '25',
        'start': '1',
        'end': '20',
      }
    ],
    'hadith': [
      {
        'translation':
            '''When carried to his grave, a dead person is followed by three, two of which return (after his burial) and one remains with him: his relative, his property, and his deeds follow him; relatives and his property go back while his deeds remain with him."
[Al Bukhari]''',
      }
    ],
    'dua': [
      {
        'arabic': '''يَا مُقَلِّبَ الْقُلُوبِ ثَبِّتْ قَلْبِى عَلَى دِينِكَ''',
        'translation':
            '''O Turner of hearts, keep my heart steadfast on Your deen.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ نَبِّهْني فيهِ لِبَرَكاتِ أسحاره، وَنَوِّرْ فيهِ قَلْبي بِضياءِ أنواره، وَخُذْ بِكُلِّ أعضائي اِلَى إتباع آثارِهِ، بِنُورِكَ يا مُنَوِّرَ قُلُوبِ الْعارِفينَ''',
        'translation':
            '''ALLAH! Make me attentive in it to the blessings of his Sahar times, enlighten my heart in it with the light of its noors, make all my parts follow its tracks with Your own Noor, O One Who brings the noor (light) to the hearts of those who know You.''',
      }
    ]
  },
  19: {
    'quran': [
      {
        'surahNumber': '25',
        'start': '21',
        'end': null,
      },
      {
        'surahNumber': '27',
        'start': '1',
        'end': '55',
      }
    ],
    'hadith': [
      {
        'translation':
            '''Whoever does not give up forged speech and evil actions, Allah is not in need of his leaving his food and drink (i.e. Allah will not accept his fasting.)
[Al Bukhari]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''رَبَّنَا ظَلَمْنَا أَنفُسَنَا وَإِن لَّمْ تَغْفِرْ لَنَا وَتَرْحَمْنَا لَنَكُونَنَّ
مِنَ الْخَاسِرِينَ''',
        'translation':
            '''Our Lord! We have wronged ourselves, and if You do not forgive us and not have
mercy on us, we shall most surely be among the losers.''',
      },
      {
        'arabic':
            '''اللهم وفر فيه حظي من بركاته وسهِّل سبيلي إلى خيراته ولا تحرمني قبول حسناته يا هادياً إلى الحق المبين''',
        'translation':
            '''O’Allah multiply my share in this month on account of its abundant blessings, make easy and smooth my means and ways to reach its mercy and bounties, and do not prevent me from getting its good, O He guides unto the clear truth''',
      }
    ]
  },
  20: {
    'quran': [
      {
        'surahNumber': '27',
        'start': '56',
        'end': null,
      },
      {
        'surahNumber': '29',
        'start': '1',
        'end': '45',
      }
    ],
    'hadith': [
      {
        'translation':
            '''When the last ten days of Ramadan began, the Prophet (صلى الله عليه وسلم) used to stay up at night, tighten his waist- wrap, and wake up his family (to pray).
[Sunan Ibn Majah]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ إِنَّكَ عَفُوٌّ تُحِبُّ الْعَفْوَ فَاعْفُ عَنِّي''',
        'translation':
            '''O Allah, You are Forgiving and love forgiveness, so forgive me.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ افْتَحْ لي فيهِ أبواب الْجِنانِ، وأغلق عَنّي فيهِ أبواب النّيرانِ، وَوَفِّقْني فيهِ لِتِلاوَةِ الْقُرْآنِ، يا مُنْزِلَ السَّكينَةِ فى قُلُوبِ الْمُؤْمِنينَ''',
        'translation':
            '''ALLAH, Open for me in it the gates of the gardens, close from me in it the gates of the fires, enable me in it to recite the Qur’an, O One Who sends down calm to the hearts of the faithful.''',
      }
    ]
  },
  21: {
    'quran': [
      {
        'surahNumber': '29',
        'start': '46',
        'end': null,
      },
      {
        'surahNumber': '33',
        'start': '1',
        'end': '30',
      }
    ],
    'hadith': [
      {
        'translation':
            '''Whoever established prayers on the night of Qadr out of sincere faith and hoping for a reward from Allah, then all his previous sins will be forgiven; and whoever fasts in the month of Ramadan out of sincere faith, and hoping for a reward from Allah, then all his previous sins will be forgiven.
[Al Bukhari]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''فَاطِرَ السَّمَاوَاتِ وَالأرْضِ أَنْتَ وَلِيِّي فِي الدُّنْيَا وَالآخِرَةِ تَوَفَّنِي
مُسْلِمًا وَأَلْحِقْنِي بِالصَّالِحِينَ''',
        'translation':
            '''Creator of the heavens and earth, You are my protector in this world and in the Hereafter. Cause me to die a Muslim and join me with the righteous.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ اجْعَلْ لى فيهِ اِلى مَرْضاتِكَ دَليلاً، وَلا تَجْعَلْ لِلشَّيْطانِ فيهِ عَلَيَّ سَبيلاً، وَاجْعَلِ الْجَنَّةَ لى مَنْزِلاً وَمَقيلاً، يا قاضِيَ حَوائِجِ الطّالِبينَ''',
        'translation':
            '''ALLAH, on this day, show me the way to win Your pleasure, do not let Shaytan have a means over me, make Paradise an abode and a resting place for me, O the One who fulfills the requests of the needy''',
      }
    ]
  },
  22: {
    'quran': [
      {
        'surahNumber': '33',
        'start': '31',
        'end': null,
      },
      {
        'surahNumber': '36',
        'start': '1',
        'end': '27',
      }
    ],
    'hadith': [
      {
        'translation':
            '''The best among you (Muslims) are those who learn the Qur'an and teach it.
[Al Bukhari]''',
      }
    ],
    'dua': [
      {
        'arabic': '''بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيمِ
قُلْ أَعُوذُ بِرَبِّ الْفَلَقِ. مِن شَرِّ مَا خَلَقَ. وَمِن شَرِّ غَاسِقٍ إِذَا وَقَبَ. وَمِن شَرِّ
النَّفَّاثَاتِ فِي الْعُقَدِ. وَمِن شَرِّ حَاسِدٍ إِذَا حَسَدَ.''',
        'translation':
            '''I seek refuge with (Allah), the Lord of the daybreak. From the evil of what He has created. And
from the evil of the darkening (night) as it comes with its darkness. And from the evil of those
who practise witchcraft when they blow in the knots, And from the evil of the envier when he
envies.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ افْتَحْ لى فيهِ أبواب فَضْلِكَ، وَاَنْزِلْ عَلَيَّ فيهِ بَرَكاتِكَ، وَوَفِّقْني فيهِ لِمُوجِباتِ مَرْضاتِكَ، وَاَسْكِنّي فيهِ بُحْبُوحاتِ جَنّاتِكَ، يا مُجيبَ دَعْوَةِ الْمُضْطَرّينَ''',
        'translation':
            '''ALLAH, on this day, open for me the doors of Your Grace, send down on me its blessings, help me towards the causes of Your mercy, and give me a place in the comforts of Paradise, O the one who answers the call of the distressed.''',
      }
    ]
  },
  23: {
    'quran': [
      {
        'surahNumber': '36',
        'start': '28',
        'end': null,
      },
      {
        'surahNumber': '39',
        'start': '1',
        'end': '31',
      }
    ],
    'hadith': [
      {
        'translation':
            '''“And when My servants ask you, [O Muhammad, concerning Me - indeed I am near. I respond to the invocation of the supplicant when he calls upon Me. So let them respond to Me [by obedience] and believe in Me that they may be [rightly] guided.”
[Al Baqarah 2:186]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''رَبِّ اغْفِرْ لِي وَتُبْ عَلَىَّ إِنَّكَ أَنْتَ التَّوَّابُ الْغَفُورُ''',
        'translation':
            '''O my Lord, forgive me, and accept my repentance. Verily, You are the Oft- Returning, the Most Forgiving.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ اغْسِلْني فيهِ مِنَ الذُّنُوبِ، وَطَهِّرْني فيهِ مِنَ الْعُيُوبِ، وَامْتَحِنْ قَلْبي فيهِ بِتَقْوَى الْقُلُوبِ، يا مُقيلَ عَثَراتِ الْمُذْنِبينَ''',
        'translation':
            '''ALLAH, on this day, wash away my sins, purify me from all flaws, examine my heart with (for) the piety of the hearts, O One who overlooks the shortcomings of the sinners.''',
      }
    ]
  },
  24: {
    'quran': [
      {
        'surahNumber': '39',
        'start': '32',
        'end': null,
      },
      {
        'surahNumber': '41',
        'start': '1',
        'end': '46',
      }
    ],
    'hadith': [
      {
        'translation':
            '''The reward of the congregational prayer is twenty seven times greater (than that of the prayer offered by a person alone).
[Al Bukhari]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ إِنِّي أَسْأَلُكَ مُوْجِبَاتِ رَحْمَتِكَ، وَعَزَائِمَ مَغْفِرَتِكَ، وَالسَّلَامَةَ مِنْ
كُلِّ إِثْمٍ، وَالْغَنِيْمَةَ مِنْ كُلِّ بِرٍ، وَالْفَوْزَ بِالجَنَّةِ، وَالنَّجَاةَ مِنَ النَّارِ''',
        'translation':
            '''O Allah! I beg You for that which incites Your mercy and the means of Your forgiveness, safety from every sin, the benefit from every good deed, success in attaining Jannah and safety from the Fire.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ إني أسألك فيهِ ما يُرْضيكَ، وأعوذ بِكَ مِمّا يُؤْذيكَ، وأسألك التَّوْفيقَ فيهِ لاِنْ أطيعك وَلا أعصيك، يا جَوادَ السّائِلينَ''',
        'translation':
            '''ALLAH, on this day, I ask You for what pleases You, and I seek refuge in You from what displeases You, I ask You to grant me the opportunity to obey You and not disobey You, O One who is generous with those who ask''',
      }
    ]
  },
  25: {
    'quran': [
      {
        'surahNumber': '41',
        'start': '47',
        'end': null,
      },
      {
        'surahNumber': '45',
        'start': '1',
        'end': null,
      }
    ],
    'hadith': [
      {
        'translation':
            '''The person who severs the bond of kinship will not enter Jannah.
[Al Bukhari]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اَللّٰهُمَّ اَلِّفْ بَيْنَ قُلُوْبِنَا وَأَصْلِحْ ذَاتَ بَيْنِنَا، وَاهْدِنَا سُبُلَ
السَّلاَمِ، وَنَجِّنَا مِنَ الظُّلُمَاتِ إِلَى النُّوْرِ''',
        'translation':
            '''O Allah, join our hearts, reform our matters within us, guide us to the path of peace, bring us from darkness to light.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ اجْعَلْني فيهِ مُحِبَّاً لأوليائك، وَمُعادِياً لأعدائك، مُسْتَنّاً بِسُنَّةِ خاتَمِ أنبيائك، يا عاصِمَ قُلُوبِ النَّبِيّينَ''',
        'translation':
            '''ALLAH, on this day, make me among those who love Your friends, and hate Your enemies, following the way of Your last Prophet, O the Guardian of the hearts of the Prophets.''',
      }
    ]
  },
  26: {
    'quran': [
      {
        'surahNumber': '46',
        'start': '1',
        'end': null,
      },
      {
        'surahNumber': '51',
        'start': '1',
        'end': '30',
      }
    ],
    'hadith': [
      {
        'translation':
            '''Whoever loves the meeting with Allah, Allah too, loves the meeting with him; and whoever hates the meeting with Allah, Allah too, hates the meeting with him.
[Al Bukhari]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ إِنِّي أَعُوذُ بِكَ أَنْ أُشْرِكَ بِكَ وَأَنَا أَعْلَمُ، وَأَسْتَغْفِرُكَ لِمَا
لَا أَعْلَمُ''',
        'translation':
            '''O Allah! I seek refuge in you lest I associate anything with you while I know it, and I seek your forgiveness for what I do not know.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ اجْعَلْ سَعْيي فيهِ مَشْكُوراً، وَذَنْبي فيهِ مَغْفُوراً وَعَمَلي فيهِ مَقْبُولاً، وَعَيْبي فيهِ مَسْتُوراً، يا اَسْمَعَ السّامِعينَ''',
        'translation':
            '''ALLAH, Make my endeavor in it appreciated (by You), my sin it forgiven, my deed in it accepted, my defect in it covered, O most hearing of those who hear (pleas).''',
      }
    ]
  },
  27: {
    'quran': [
      {
        'surahNumber': '51',
        'start': '31',
        'end': null,
      },
      {
        'surahNumber': '57',
        'start': '1',
        'end': '29',
      }
    ],
    'hadith': [
      {
        'translation':
            '''He who recites سُبْحَانَ ال لَّهِ وَبِحَمْدِهِ in the morning and evening 100 times, will not be surpassed on the Day of Judgement by anyone with better deeds except the one who says the same words or more.
[Muslim]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''رَبَّنَا آتِنَا فِي الدُّنْيَا حَسَنَةً وَفِي الْآخِرَةِ حَسَنَةً وَقِنَا عَذَابَ
النَّار''',
        'translation':
            '''Our Lord, give us in this world [that which is] good and in the Hereafter [that which is] good and protect us from the punishment of the Fire.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ ارْزُقْني فيهِ فَضْلَ لَيْلَةِ الْقَدْرِ، وَصَيِّرْ أموري فيهِ مِنَ الْعُسْرِ إلى الْيُسْرِ، وَاقْبَلْ مَعاذيري، وَحُطَّ عَنّيِ الذَّنْبَ وَالْوِزْرَ، يا رَؤوفاً بِعِبادِهِ الصّالِحينَ''',
        'translation':
            '''ALLAH, on this day, bestow on me the blessings of Laylatul Qadr, change my affairs from (being) difficult to (being) easy, accept my apologies, and decrease for me [my] sins and burdens, O the Compassionate with His righteous servants.''',
      }
    ]
  },
  28: {
    'quran': [
      {
        'surahNumber': '58',
        'start': '1',
        'end': null,
      },
      {
        'surahNumber': '66',
        'start': '1',
        'end': '12',
      }
    ],
    'hadith': [
      {
        'translation':
            '''Verily, when the slave (of Allah) commits a sin, a black spot appears on his heart. When he refrains from it, seeks forgiveness and repents, his heart is polished clean.
[Tirmidhi]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ رَحْمَتَكَ أَرْجُو فَلَا تَكِلْنِي إِلَى نَفْسِي طَرْفَةَ عَيْنٍ أَصْلِحْ
لِي شَأْنِي كُلَّهُ لَا إِلَهَ إِلَّا أَنْت''',
        'translation':
            '''O Allah I hope for Your Mercy, do not entrust me to my own self even for the blink of an eye, correct for me all my matters, there is none worthy of worship except You.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ وَفِّرْ حَظّي فيهِ مِنَ النَّوافِلِ، وأكرمني فيهِ بإحضار الْمَسائِلِ، وَقَرِّبْ فيهِ وسيلتي إليك مِنْ بَيْنِ الْوَسائِلِ، يا مَنْ لا يَشْغَلُهُ إلحاح الْمُلِحّينَ''',
        'translation':
            '''ALLAH, on this day, grant me a share in its nawafil (recommended prayers), honor me by attending to my problems, make closer the means to approach You, from all the means, O One who is not preoccupied by the requests of the beseechers.''',
      }
    ]
  },
  29: {
    'quran': [
      {
        'surahNumber': '67',
        'start': '1',
        'end': null,
      },
      {
        'surahNumber': '77',
        'start': '1',
        'end': null,
      }
    ],
    'hadith': [
      {
        'translation':
            '''He whose last words are لاَ إِلَهَ إِلاَّ اللَّهُ will enter Paradise.
[Abu Dawood]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''رَبَّنَا لاَ تُزِغْ قُلُوبَنَا بَعْدَ إِذْ هَدَيْتَنَا وَهَبْ لَنَا مِن لَّدُنكَ رَحْمَةً
إِنَّكَ أَنتَ الْوَهَّابُ''',
        'translation':
            '''Our Lord! Let not our hearts deviate after You have guided us, and grant us mercy from You. Truly, You are the One Who Bestows.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ غَشِّني فيهِ بِالرَّحْمَةِ ، وَارْزُقْني فيهِ التَّوْفيقَ وَالْعِصْمَةَ ، وَطَهِّرْ قَلْبي مِنْ غَياهِبِ التُّهْمَةِ ، يا رَحيماً بِعِبادِهِ الْمُؤْمِنينَ''',
        'translation':
            '''O ALLAH, on this day, cover me with Your mercy, grant me in it success and protection, purify my heart from the darkness of false accusations, O the Merciful to His believing servants.''',
      }
    ]
  },
  30: {
    'quran': [
      {
        'surahNumber': '78',
        'start': '1',
        'end': null,
      },
      {
        'surahNumber': '144',
        'start': '1',
        'end': null,
      }
    ],
    'hadith': [
      {
        'translation':
            '''Whoever observes the fast of Ramadan and follows it up with six fasts in Shawwal will be rewarded as if he has fasted the entire year.
[Muslim]''',
      }
    ],
    'dua': [
      {
        'arabic':
            '''اللَّهُمَّ بَاعِدْ بَيْنِي وَبَيْنَ خَطَايَاىَ كَمَا بَاعَدْتَ بَيْنَ الْمَشْرِقِ وَالْمَغْرِبِ
اللَّهُمَّ نَقِّنِي مِنْ خَطَايَاىَ كَمَا يُنَقَّى الثَّوْبُ الأَبْيَضُ مِنَ الدَّنَسِ
اللَّهُمَّ اغْسِلْنِي مِنْ خَطَايَاىَ بِالثَّلْجِ وَالْمَاءِ وَالْبَرَدِ''',
        'translation':
            '''O Allah, remove my sins from me as Thou hast removed the East from the West. O Allah purify
me from sins as a white garment is purified from filth. O Allah! wash away my sins with snow,
water and ice.''',
      },
      {
        'arabic':
            '''اَللّـهُمَّ اجْعَلْ صِيامى فيهِ بِالشُّكْرِ وَالْقَبُولِ عَلى ما تَرْضاهُ وَيَرْضاهُ الرَّسُولُ، مُحْكَمَةً فُرُوعُهُ بِالاْصُولِ، بِحَقِّ سَيِّدِنا مُحَمَّد وَآلِهِ الطّاهِرينَ، وَالْحَمْدُ للهِ رَبِّ الْعالَمينَ''',
        'translation':
            '''O ALLAH, on this day, make my fasts worthy of appreciation and acceptance, according to what pleases You, and pleases the Messenger, the branches being strengthened by the roots, for the sake of our leader, Muhammad, and his purified family. Praise be to ALLAH, the Lord of the worlds.''',
      }
    ]
  },
};
